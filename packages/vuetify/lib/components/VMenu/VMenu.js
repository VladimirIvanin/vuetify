import { h, Transition, vShow, withDirectives } from 'vue'; // Styles
...
export default baseMixins.extend({
  ...
  computed: {
    ...
    calculatedMaxHeight() {
      const height = this.auto ? '220px' : convertToUnit(this.maxHeight);
      return height || '0';
    },
  ...
  methods: {
    ...
    genDirectives() {
      const directives = [[vShow, this.isContentActive]]; // Do not add click outside for hover menu
    ...
    genContent() {
      const options = { ...this.getScopeIdAttrs(),
        ...this.contentProps,
  ...
  render() {
    ...
    const directives = [[Resize, this.onResize, '500']]; // console.log('vmenurender', this, this.activator, this.getActivator())

    return withDirectives(h('div', data, [!this.activator && this.genActivator(), this.showLazyContent(() => [h(VThemeProvider, {
      root: true,
      light: this.light,
      dark: this.dark
    }, [this.genTransition()])])]), directives);
  }
