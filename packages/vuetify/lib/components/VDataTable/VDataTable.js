import "../../../src/components/VDataTable/VDataTable.sass"; // Types
...
import Mouse from '../../mixins/mouse'; // Helpers
...
export default mixins(VDataIterator, Loadable, Mouse).extend({
  ...
  methods: {
    ...
    createItemProps(item, index) {
      const data = { ...VDataIterator.methods.createItemProps.call(this, item, index),
    ...
    genHeaders(props) {
      const data = { ...this.sanitizedHeaderProps,
        headers: this.computedHeaders,
      ...
      if (!this.hideDefaultHeader) {
        ...
        children.push(h(VDataTableHeader, { ...data
        }, scopedSlots));
      }
    ...
    genScopedRows(items, props) {
      ...
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        rows.push(this.$slots.item({ ...this.createItemProps(item, i),
          isMobile: this.isMobile
        }));
    ...
    genDefaultRows(items, props) {
      return this.$slots['expanded-item'] ? items.map((item, index) => this.genDefaultExpandedRow(item, index)) : items.map((item, index) => this.genDefaultSimpleRow(item, index));
    },
    ...
    genBody(props) {
      ...
      return h('tbody', [getSlot(this, 'body.prepend', data, true), this.genItems(props.items, props), getSlot(this, 'body.append', data, true)]);
    },
    ...
    genFooters(props) {
      ...
      const children = [getSlot(this, 'footer', { ...data,
        widths: this.widths,
        headers: this.computedHeaders
      }, true)];

      if (!this.hideDefaultFooter) {
        children.push(h(VDataFooter, { ...data
    ...
    genDefaultScopedSlot(props) {
      ...
      return h(VSimpleTable, { ...simpleProps,
        class: {
          'v-data-table--mobile': this.isMobile,
          'v-data-table--selectable': this.showSelect
        }
      }, {
        default: () => [this.genCaption(props), this.genColgroup(props), this.genHeaders(props), this.genBody(props), this.genFoot(props)],
        top: () => getSlot(this, 'top', { ...props,
          isMobile: this.isMobile
        }, true),
