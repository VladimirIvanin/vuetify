...
/* @vue/component */
  ...
  methods: {
    ...
    setPicker(picker) {
      if (picker === 'HOUR') this.selecting = SelectingTimes.Hour;else if (picker === 'MINUTE') this.selecting = SelectingTimes.Minute;else if (picker === 'SECOND' && this.useSeconds) {
        this.selecting = SelectingTimes.Second;
      }
    },
    ...
    setInputData(value) {
      if (value == null || value === '') {
        ...
        const match = value.trim().match(/^(\d+):(\d+)(:(\d+))?\s*([ap]m)?$/i);
    ...
    genClock() {
      return h(VTimePickerClock, {
        ...
        modelValue: this.selecting === SelectingTimes.Hour ? this.inputHour : this.selecting === SelectingTimes.Minute ? this.inputMinute : this.inputSecond,
