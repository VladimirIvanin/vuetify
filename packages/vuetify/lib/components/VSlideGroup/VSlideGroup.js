...
export const BaseSlideGroup = mixins(BaseItemGroup, Mobile).extend({
  ...
  methods: {
    ...
    genNext() {
      const slot = this.$slots.next ? this.$slots.next({}) : getSlot(this, 'next') || this.__cachedNext;
      return h('div', {
    ...
    genPrev() {
      const slot = this.$slots.prev ? this.$slots.prev({}) : getSlot(this, 'prev') || this.__cachedPrev;
      return h('div', {
  ...
  render() {
    return withDirectives(h('div', this.genData(), [this.genPrev(), this.genWrapper(), this.genNext()]), [[Resize, this.onResize]]);
  }
