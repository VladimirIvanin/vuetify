...
/* @vue/component */
  ...
  emits: ['update:modelValue', 'change', 'update:active-picker', 'update:period', 'click:hour', 'click:minute', 'click:second'],
  data: function data() {
    return {
      inputHour: null,
      inputMinute: null,
  ...
  methods: {
    ...
    emitPicker: function emitPicker(value) {
      ...
      if (value === _SelectingTimes.SelectingTimes.Minute) {
        activePicker = 'MINUTE';
      } else if (value === _SelectingTimes.SelectingTimes.Second) {
        activePicker = 'SECOND';
      }
    ...
    setPicker: function setPicker(picker) {
      if (picker === 'HOUR') this.selecting = _SelectingTimes.SelectingTimes.Hour;else if (picker === 'MINUTE') this.selecting = _SelectingTimes.SelectingTimes.Minute;else if (picker === 'SECOND' && this.useSeconds) {
        this.selecting = _SelectingTimes.SelectingTimes.Second;
      }
    },
    ...
    setInputData: function setInputData(value) {
      if (value == null || value === '') {
        ...
        var match = value.trim().match(/^(\d+):(\d+)(:(\d+))?\s*([ap]m)?$/i);
        ...
        if (match) {
          ...
          var normalizedPeriod = period ? period.toLowerCase() : null;
          this.inputHour = normalizedPeriod ? this.convert12to24(parseInt(hour, 10), normalizedPeriod) : parseInt(hour, 10);
          this.inputMinute = parseInt(minute, 10);
          this.inputSecond = parseInt(second || 0, 10); // Устанавливаем период только если он был указан в строке
    ...
    genClock: function genClock() {
      return (0, _vue.h)(_VTimePickerClock.default, {
        ...
        modelValue: this.selecting === _SelectingTimes.SelectingTimes.Hour ? this.inputHour : this.selecting === _SelectingTimes.SelectingTimes.Minute ? this.inputMinute : this.inputSecond,
    ...
    genClockAmPm: function genClockAmPm() {
      return (0, _vue.h)('div', this.setTextColor(this.color || 'primary', {
    ...
    genPickerTitle: function genPickerTitle() {
      ...
      return (0, _vue.h)(_VTimePickerTitle.default, {
        ampm: this.isAmPm,
        ...
        'onUpdate:selecting': function onUpdateSelecting(value) {
          _this4.selecting = value;
        },
        'onUpdate:period': function onUpdatePeriod(period) {
          _this4.$emit('update:period', period);
        },
        ref: 'title'
      });
    }
